...
export function AppearanceSettingsCard() {
  ...
  return (
    <div className="studio-card p-6 space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-ivory">Appearance</h3>
        <p className="text-xs text-ivory-muted mt-1">Choose light or dark theme.</p>
      </div>
            ...
            <button
              ...
              className={cn(
                ...
                active
                  ...
                  : 'border-slate hover:border-ivory-muted bg-marble',
              )}
            >
              <div
                className={cn(
                  'h-14 rounded-lg mb-3 border border-slate',
                  mode === 'dark' ? 'bg-black' : 'bg-[#f5f5f7]',
                )}
              />
              <p className="text-sm font-semibold text-ivory capitalize">{mode}</p>
              <p className="text-[11px] text-ivory-muted mt-0.5">
                {mode === 'dark' ? 'Black background' : 'White background'}
              </p>
            </button>
          );
        })}
